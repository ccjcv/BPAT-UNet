...
class DeformableAttention2D(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.pos_gate = nn.Parameter(torch.tensor(1.0), requires_grad=True)
    ...
    def forward(self, x, return_vgrid = False):
        ...
        # v_1 = self.m1(v)#(16,512,1,1)
        # v_1 = v_1.view(b, 512, -1)
        # v_2 = self.m2(v)#(16,512,2,2)
        # v_2 = v_2.view(b, 512, -1)
        # v_3 = self.m3(v)#(16,512,4,4)
        # v_3 = v_3.view(b, 512, -1)
        # v = torch.cat((v_1, v_2, v_3), 2)#(16, 512, 21)
        #
        # k_1 = self.m1(k)  # (16,512,1,1)
        # k_1 = k_1.view(b, 512, -1)
        # k_2 = self.m2(k)  # (16,512,2,2)
        # k_2 = k_2.view(b, 512, -1)
        # k_3 = self.m3(k)  # (16,512,4,4)
        # k_3 = k_3.view(b, 512, -1)
        # k = torch.cat((k_1, k_2, k_3), 2)  # (16, 512, 21)
