...
class RandomHorizontalFlip(object):
    def __call__(self, sample):
        ...
        if random.random() < 0.5:
            ...
            point = point.transpose(Image.FLIP_LEFT_RIGHT)
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        point = np.array(sample['point']).astype(np.float32)
        img /= 255.0
        img -= self.mean
        img /= self.std
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        point = np.expand_dims(np.array(sample['point']).astype(np.float32), -1).transpose((2, 0, 1))
...
class FixedResize(object):
    ...
    def __call__(self, sample):
        ...
        point = point.resize(self.size, Image.NEAREST)
